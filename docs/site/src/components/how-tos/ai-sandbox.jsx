    ...
    <HowToPage title="AI sandbox box">
      ...
      <div className="card">
        <p>
          ...
          tool pre-installed. Experiment on your outpost instead of your laptop — isolated from
      ...
      <div className="card">
        ...
        <p>
          ...
          <code>outpost template new {'<name>'} [command]</code>
        </p>
        <p className="note">
          Search by name: <code>outpost template search codex</code>. Combine templates
      ...
      <div className="card">
        ...
        <pre>
          <code>outpost create sandbox --template opencode claude-code</code>
        </pre>
        <p>
          outpost launches an EC2 instance, and runs the template install scripts on first boot.
        </p>
        <p className="note">
          New to outpost? See <Link to="/docs/install">Installation</Link> and{' '}
      ...
      <div className="card">
        ...
        <pre>
          <code>{`outpost status sandbox
outpost ssh sandbox`}</code>
        </pre>
        <p>
          <code>outpost ssh</code> waits until the instance is ready and templates have
...
codex`}</code>
        ...
        <ul>
          <li>
            Use <code>outpost git-sync sandbox</code> to forward your local GitHub SSH key
            for <code>git clone</code> / <code>git push</code> without copying keys to
            the box.
          </li>
          <li>
            Copy local files with <code>outpost cp</code> or <code>outpost sync</code> — see{' '}
            <Link to="/how-tos/transfer">Transfer data and files</Link>.
          </li>
          <li>
            Open the project in VS Code via Remote-SSH to <code>outpost-sandbox</code> for
      ...
      <div className="card">
        ...
        <pre>
          <code>outpost stop sandbox</code>
        </pre>
        <p>Or auto-stop after inactivity:</p>
        <pre>
          <code>outpost idle-stop set sandbox 60</code>
        </pre>
        <p>
          Snapshot a box you want to reuse later:{' '}
          <code>outpost snapshot create sandbox my-snapshot</code>, then restore with{' '}
          <code>outpost create newbox --template codex22 --from my-snapshot</code>.
        </p>
      </div>
    </HowToPage>
  )
